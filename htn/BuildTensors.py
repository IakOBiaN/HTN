...
def build_matrix(calc, temp, m_par):
	...
	matrixes = []
	if model == "mono":
		matrixes = [np.array([[0.0, m_par[0] / neighbours], [m_par[0] / neighbours, -m_par[1] + m_par[0] / (neighbours / 2.0)]]) ,] * 3
	elif model == "binary":
		#m_par: 0 - muA, 1 - muB, 2 - epsAA, 3 - epsBB, 4 - epsAB
		matrixes = [np.array([[0.0, m_par[0] / neighbours, m_par[1] / neighbours], [m_par[0] / neighbours, -m_par[2] + 2.0 * m_par[0] / neighbours, (m_par[0] + m_par[1]) / neighbours], [m_par[1] / neighbours, (m_par[0] + m_par[1]) / neighbours, -m_par[3] + 2.0 * m_par[1] / neighbours]]) ,] * 3
	elif model == "ising":
		matrixes = [np.array([[(m_par[1] - m_par[0] / (neighbours / 2.0)), (-m_par[1])],[(-m_par[1]), (m_par[1] + m_par[0] / (neighbours / 2.0))]]), ] * 3
